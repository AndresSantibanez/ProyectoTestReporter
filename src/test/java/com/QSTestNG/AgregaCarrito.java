	...
	public void continuarCompra() throws InterruptedException {
		try {
			...
			if(sBL.contentEquals("search_query_top")) {
				System.out.println("searchBox Pass");
			...
			if(aL.contains("product-image-container")) {
				...
				FileHandler.copy(scrFile, new File(System.getProperty("user.dir") + "\\acArticulo.png"));	
			}else {
				System.out.println("No se ha encontrado el objeto....");
				d.quit();
			}
...
//			Thread.sleep(1000);
			...
			String add = d.findElement(btnAddLocator).getAttribute("name");
			if(add.contains("Submit")) {
				File srcFile = d.findElement(btnAddLocator).getScreenshotAs(OutputType.FILE);
				FileHandler.copy(srcFile, new File(System.getProperty("user.dir") + "\\btnAgregar.png"));
			...
			if (titulo.isDisplayed()) {
				...
				FileHandler.copy(scrFile2, new File(System.getProperty("user.dir") + "\\tituloOk.png"));
			} else {
				System.out.println("no se ha encontrado el titulo: "+titulo);
			}
		} catch (Exception e) {
